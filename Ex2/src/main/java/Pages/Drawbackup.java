...
public class Drawbackup extends JFrame{
    Graph graph;
    int width=1000;
    int height=700;
    Point3D points[];
    ArrayList<Point3D> point;
    public Drawbackup(Graph graph){
    ...
    public void paint(Graphics g) {
        ...
        while (iteratorEdges.hasNext()){
            ...
            Line2D.Double line = new Line2D.Double(point.get(point.indexOf(src.getKey())).x(),point.get(point.indexOf(src)).y(),points[dest.getKey()].x(),points[dest.getKey()].y());
            g2d.draw(line);
